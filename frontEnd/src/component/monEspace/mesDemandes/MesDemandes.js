...
import './MesDemandesStyle.css'
import Accordion from 'react-bootstrap/Accordion'
import Card from 'react-bootstrap/Card';
import Button from 'react-bootstrap/Button';
import { MDBInput } from 'mdbreact';
import { addDemandes, removeDemandes } from '../../../actions/actionsServices ';
...
class MesDemandes extends Component {
    ...
    handleChangeDemandes = (e) => {
        this.setState({ demandes: e.target.value })
    }
    handleDiscription = (e) => {
                    ...
                    <Card>
                        <Card.Header>
                            <div className="d-flex justify-content-between  align-items-center" >
                                <div className="panel-heading">DEMANDES QUE JE PEUX RENDRE</div>
                            ...
                            <table className="table ">
                                ...
                                <tbody>
                                    ...
                                    {
                                        this.props.allState.demandes.map(el =>
                                                <tr>
                                                    <td> {el.demandes} </td>
                                                    <td> {el.discription} </td>
                                                    <td>
                                                        <img onClick={() => this.props.deleteDemandes(el.id)} src="http://www.yakasaider.fr/images/admin/trash.png" alt="" title="Supprimer" border="0" />
                                                    </td>
                                                </tr>
                                            )
                                    }
                        ...
                        <Accordion.Collapse eventKey="0">
                            <Card.Body>Type de Service :

                                <select onChange={this.handleChangeDemandes} id="demandes" name="catid" className="form-control bs-select-hidden" title="Catégories" data-live-search="true" data-header="Choisir une catégorie">
                                    <option className="bs-title-option" value="">Tous les Service</option>
                                ...
                                <MDBInput onChange={this.handleDiscription} type="textarea" label="Rédigez votre demande" outline />
                                <div className="d-flex justify-content-end">
                                    <button onClick={() => this.props.addNewDemandes(  {...this.state , id : uuid()}  )} className="outline-primary" color="primary"><strong>Valider</strong></button>
                                </div>
...
const mapDispatchToProps = dispatch => {
    return {
        addNewDemandes: y => dispatch(addDemandes(y)),
        deleteDemandes: id => dispatch(removeDemandes(id))
    }
}
...
export default connect(mapStateToProps, mapDispatchToProps)(MesDemandes);